...
from services.document_schema import DOCUMENT_SCHEMA_FILE_NAME
from services.document_schema import DOCUMENT_SCHEMA_REPORT_FILE_NAME
from services.document_schema import PROVIDER_GENERIC_FLAT_OCR
...
from services.ocr_provider.qwen_ocr import convert_pdf as run_qwen_ocr
from services.ocr_provider.qwen_ocr import DEFAULT_BASE_URL as QWEN_OCR_DEFAULT_BASE_URL
from services.ocr_provider.qwen_ocr import DEFAULT_MODEL as QWEN_OCR_DEFAULT_MODEL
...
def _args_from_spec(spec: ProviderStageSpec) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        qwen_token=provider_token if provider == "qwen" else "",
        model_version=spec.ocr.model_version,
        qwen_base_url=spec.ocr.qwen_base_url,
        qwen_model=spec.ocr.qwen_model,
        qwen_workers=spec.ocr.qwen_workers,
...
def run_qwen_to_job_dir(args: SimpleNamespace) -> tuple[Path, Path, Path, Path]:
    job_dirs = job_dirs_from_explicit_args(args)
    source_pdf_path = Path(str(args.file_path or "")).resolve()
    if not source_pdf_path.exists():
        source_url = str(args.file_url or "").strip()
        if not source_url:
            raise RuntimeError("Qwen OCR requires a local PDF or source URL")
        source_pdf_path = _download_source_pdf(source_url, job_dirs.source_dir)

    provider_result_json_path = job_dirs.ocr_dir / "qwen_result.json"
    normalized_json_path = job_dirs.ocr_dir / DOCUMENT_SCHEMA_FILE_NAME
    normalized_report_json_path = job_dirs.ocr_dir / DOCUMENT_SCHEMA_REPORT_FILE_NAME
    model = str(args.qwen_model or QWEN_OCR_DEFAULT_MODEL).strip()
    workers = int(args.qwen_workers or 3)
    if workers < 1 or workers > 8:
        raise RuntimeError("qwen_workers must be between 1 and 8")

    run_qwen_ocr(
        source_pdf_path,
        provider_result_json_path,
        api_key=str(args.qwen_token or "").strip(),
        base_url=str(args.qwen_base_url or QWEN_OCR_DEFAULT_BASE_URL).strip(),
        model=model,
        dpi=144,
        start_page=0,
        end_page=-1,
    ...
    document, report = adapt_path_to_document_v1_with_report(
        ...
        document_id=source_pdf_path.stem,
        provider_version=model,
    )
    save_json(normalized_json_path, document)
    save_json(normalized_report_json_path, report)
    validate_saved_document_path(normalized_json_path)
    return job_dirs, source_pdf_path, provider_result_json_path, normalized_json_path
...
def main() -> None:
    ...
    with pipeline_event_writer_scope(event_writer):
        ...
        if provider == "mineru":
            ...
            emit_stage_transition(
                stage="ocr_processing",
                message="Starting Qwen OCR provider pipeline",
                provider=provider,
            )
            _, source_pdf_path, layout_json_path, normalized_json_path = run_qwen_to_job_dir(args)
            job_dirs = job_dirs_from_explicit_args(args)
        else:
